...
def listar_classes(pasta_dataset: str | Path) -> list[tuple[str, list[Path]]]:
    ...
    if len(classes) < 2:
        raise ValueError("São necessárias pelo menos duas pastas com fotos, por exemplo ")
    
    return classes

def extrair_dataset(detector, classes: list[tuple[str, list[Path]]], minimo_por_classe: int) -> dict[str, np.ndarray]:
    dados = {}
...
def criar_modelo(X_normalizacao: np.ndarray, numero_classes: int) -> keras.Model:
    ...
    modelo.compile(optimizer=keras.optimizers.Adam(learning_rate=1e-3), loss="sparse_categorical_crossentropy", metrics=["accuracy"])
    return modelo
...
def separar_validacao(dados: dict[str, np.ndarray]):
    ...
    return (np.asarray(X_treino, dtype=np.float32),
        np.asarray(y_treino),
...
def salvar_relatorio_visual(
    ...
    classes: list[str],
    acuracia: float, caminho: str | Path) -> None:
    ...
    exibicao.plot(ax=eixo_matriz, cmap="Blues", include_values=False, colorbar=True)
    eixo_matriz.set_title("Matriz de confusão normalizada por letra")
    eixo_matriz.tick_params(axis="x", labelrotation=45)
    figura.suptitle(f"Validação acurácia: {acuracia * 100:.2f}%",fontsize=16,)
    figura.tight_layout(rect=(0, 0, 1, 0.96))
    figura.savefig(caminho, dpi=160, bbox_inches="tight")
    plt.close(figura)

def verificar_qualidade(acuracia: float, metricas: dict,classes: list[str], acuracia_minima: float,recall_minimo: float) -> None:
    ...
    if acuracia < acuracia_minima:
        motivos.append(f"acurácia {acuracia * 100:.2f}% abaixo de {acuracia_minima * 100:.2f}%")
    if classes_com_recall_baixo:
        motivos.append("recall abaixo do mínimo em: " + ", ".join(classes_com_recall_baixo))
...
def treinar(args: argparse.Namespace) -> None:
    if args.sem_validacao:
        raise ValueError("Sem validacao foi desativado para evitar salvar um modelo sem avaliação.")
    ...
    print(f"\nValidando com {len(X_treino)} amostras de treino e {len(X_val)} amostras separadas...")
    ...
    metricas = classification_report(y_val, previsoes, labels=classes_ordenadas, zero_division=0, output_dict=True)
    print(f"Acurácia: {acuracia * 100:.2f}%")
    print(classification_report(y_val, previsoes, labels=classes_ordenadas, zero_division=0, digits=3))
    ...
    verificar_qualidade(acuracia, metricas, classes_ordenadas, args.acuracia_minima, args.recall_minimo,)
...
parser = argparse.ArgumentParser(description="Extrai os pontos das fotos e treina o modelo de Libras.")
...
parser.add_argument("--acuracia-minima", type=probabilidade,default=0.90)
parser.add_argument("--recall-minimo", type=probabilidade, default=0.70)
